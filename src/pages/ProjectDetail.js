...
function ProjectDetail() {
  ...
  const fetchProjectData = () => {
    ...
    ProjectAPI.getProject(projectId).then((data) => {
      ...
      if (matchingProjects.length > 0) {
        ...
        setSkillsRequired(
          project.skillsRequired.reduce((skillReq, skill) => {
  ...
  const [imageURL, setImageURL] = useState(
    "https://i.ibb.co/rxmfKsL/buissines-3.png"
  );
    ...
    <div className="main-content">
      <Grid item xs={12} md={6}>
        <Typography variant="h5" component="div" color="red">
          อยู่ระหว่างการจัดทำ Project id : {id}
        </Typography>
        <Box
          sx={{
            width: "100%",
        ...
        >
          ...
          >
            ...
            <Select
              ...
              onChange={(event) => {
                ...
                setDisplayedSkills(
                  skillList.filter(
                    (s) => currentType === "" || s.skillType === currentType
                  )
                );
          ...
          >
            {skillList
              .filter((skill) => skillsRequired[skill.skill_id])
              .map((item) => (
                <Chip
                  sx={{
                    m: 1,
          ...
          <span
            style={{ fontSize: "15px", marginLeft: "10px", marginTop: "10px" }}
          >
            Project Member
          </span>
          <div
